...
| `paid_minor` | the wallet part plus what the provider actually settled. An order the wallet covers in full is born with it equal to `wallet_minor`, and with `paid_at` set, at creation |
...
| → `paid` at creation | order creation | `external_minor = 0`, the wallet covered everything; `paid_minor` and `paid_at` are written in the same insert |
