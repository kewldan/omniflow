...
matters when you go looking for it — see [the dead-letter path](#the-dead-letter-path).
...
<Warning>
  **The panel's Retry moves the row but does not re-run it.** `POST /v1/panel/system/jobs/{id}/retry`
  sets the operation back to `pending` and writes an audit event; nothing re-inserts a queue job, and
  no scanner picks operations up by status. A retried operation therefore waits until the
  fifteen-minute reconciliation sweep happens to cover that entitlement — which creates a *different*
  operation — rather than being re-attempted itself. The retry that actually re-runs a job is
