...
| `create` (known remote user) | `PATCH /api/users/`; a `404` falls back to the create-or-recover path. Then, for an existing user: `POST /api/users/{id}/actions/reset-traffic` when the operation asks for it, `POST /api/users/{id}/actions/enable` when Remnawave holds the user `DISABLED`, and a re-read |
...
| `reconcile` | Compare first, then the same `PATCH` as `create`, without the reset or the enable |
...
Every `PATCH` and `POST /api/users/` carries both limits explicitly — `trafficLimitBytes` as `0`
and `hwidDeviceLimit` as `null` when the plan has none — because Remnawave reads an absent field as
"leave it as it is". Omitting them was how a move to an unlimited plan left the previous plan's cap
on the user.

### What a paid change does beyond the expiry

Settlement marks the desired state of an `extension`, a `renewal`, and an `upgrade` with
`resetTraffic`, and the worker resets the user's traffic counter once the new state is applied: the
customer paid for a new period, and a user Remnawave holds `LIMITED` on last period's usage comes
back `ACTIVE`. A `purchase` provisions a new user with nothing to reset; a `downgrade` keeps the
current period's count.

A `create` against a user Remnawave holds `DISABLED` re-enables it, unless the entitlement is
`paused` — a pause is a deliberate disable whose clock Omniflow stopped, and only a resume may lift
it. This is what makes a paid renewal of a user that expired and was then disabled actually restore
access rather than only move its expiry. A `reconcile` does neither: it is a repair of the state the
customer paid for, not a new purchase, and an operator's `disable` must survive it.
...
An operation whose entitlement has since been `superseded` is cancelled before Remnawave is asked
anything, with `last_error_code = "entitlement_superseded"` and a `cancelled` history row. This is
the guard against a stale reconcile: a `reconcile` queued for the old entitlement and run after the
renewal's `create` would otherwise push the old expiry over the new one and write `active` back
onto a retired row. It is not a failure and notifies nobody — the work is simply no longer wanted.
...
- An entitlement whose `ends_at` is more than **thirty days** in the past is not selected either.
  Remnawave expired the user on its own at the pushed expiry, and the only thing a reconcile could
  still do is re-push a date that has passed or recreate a user an operator deleted on purpose —
  every fifteen minutes, forever. Thirty days outlasts the grace and recovery windows, after which
  the row is history rather than state; a customer who returns later buys a new entitlement, which
  is provisioned by its own `create`.
- `reconciled_at` is stamped only after a successful apply, so a permanently failing entitlement is
  re-queued in every bucket and produces one new operation row per fifteen minutes, until it falls
  past the horizon.

The interval, the batch size, and the horizon are hard-coded. There is no environment variable for
any of them.
...
| `expiry` | The remote expiry instant differs from the desired end plus the plan's grace period — the same instant the worker pushes |
| `traffic` | The remote traffic limit differs from the allowance, where no allowance means a remote limit of `0` |
| `device_limit` | The remote device limit differs from the configured one, where no configured limit means a remote `null` or `0` |
...
size (500), its horizon (30 days past the end), the revival scan's interval (5 minutes) and stale
window (10 minutes), the queue's attempt ceiling (12), the backoff ceiling (1280 seconds), the operator-alert
