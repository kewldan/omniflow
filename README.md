...
> [!IMPORTANT]
> Omniflow is under active development. The platform foundation, the commerce backend, the complete Telegram customer product, production runtime, the v0.6 operator panel foundation, and the v0.7 operator workspace — dashboard, customer and subscription administration, catalogue, finance with CSV export, job and webhook diagnostics, risk review, and commerce settings — are implemented and verified against PostgreSQL 18. Digital goods deliver through a durable worker that refunds a permanent failure to the customer's wallet and parks an outcome nobody can safely resolve for operator review, and the periodic passes behind blocklist refresh, anomaly evaluation, and gift expiry run on their own schedule. What is still missing from gifts, the shop, and recurring payments is the customer-facing half — the Telegram flows, and the worker that attempts a renewal charge — so nothing creates such an order yet; those items are unchecked in the roadmap. The support desk and campaign delivery arrive in v0.8. Web checkout remains intentionally disabled until the customer panel milestone; a payment method is offered in the bot only when the operator has configured that adapter.
