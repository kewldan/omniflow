...
> [!IMPORTANT]
> Omniflow is under active development. The platform foundation, the commerce backend, the complete Telegram customer product, production runtime, the v0.6 operator panel foundation, and the v0.7 operator workspace are implemented and verified against PostgreSQL 18. An operator can read the dashboard with its incidents and required actions, administer customers and subscriptions, publish plan versions and add-ons, target a personal offer, refund an order or re-poll a stuck payment, configure a payment provider and test its credentials, run a previewed bulk action with per-item results, and import or export customers. Customers can buy a gift, redeem one, shop for Telegram Premium and Stars, and turn on automatic renewal from their wallet or a saved card; a renewal worker charges on a bounded dunning schedule and hands the customer back to manual renewal rather than escalating. Promo codes and saved carts do not yet extend to shop orders, which is the one v0.7 item left open and is a catalogue design decision rather than missing wiring. The support desk and campaign delivery arrive in v0.8. Web checkout remains intentionally disabled until the customer panel milestone; a payment method is offered in the bot only when the operator has configured that adapter.
